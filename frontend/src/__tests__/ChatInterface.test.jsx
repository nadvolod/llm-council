...
function makeFile(name, { size = 100, type = 'text/plain' } = {}) {
...
describe('ChatInterface', () => {
  ...
  describe('form visibility (regression for hidden-after-first-message bug)', () => {
    ...
    it('still renders input form after a user message exists', () => {
      ...
      render(
        <ChatInterface conversation={conv} onSendMessage={onSendMessage} isLoading={false} />
      );
      expect(screen.getByPlaceholderText(/Ask your question/i)).toBeInTheDocument();
    });
  });
  ...
  describe('sending messages', () => {
    it('calls onSendMessage with text and empty file list', async () => {
      ...
      await user.type(screen.getByPlaceholderText(/Ask your question/i), 'Hi there');
  ...
  describe('file upload', () => {
    ...
    it('rejects unsupported extension with inline error', () => {
      ...
      // No chips should have been added (file appears only in the alert).
    ...
    it('rejects file larger than 10 MB', () => {
      ...
      selectFiles([makeFile('huge.pdf', { size: 11 * 1024 * 1024, type: 'application/pdf' })]);
    ...
    it('sends files alongside text on submit and clears state', async () => {
      ...
      selectFiles([file]);
      await user.type(screen.getByPlaceholderText(/Ask your question/i), 'analyze');
      await user.click(screen.getByRole('button', { name: /^Send$/i }));
